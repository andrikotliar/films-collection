...
const getBoxOfficeTargetValue = (
  ...
  if (boxOffice) {
    ...
    if (
      ...
      boxOffice < TARGET_VALUES.mid.value
    ) {
      return TARGET_VALUES.mid;
    }

    if (boxOffice > TARGET_VALUES.max.value) {
      const nearestBillion = Math.ceil(
        boxOffice / TARGET_VALUES.max.value,
      );
      const value =
        nearestBillion * TARGET_VALUES.max.value;
